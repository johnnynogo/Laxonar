...
void waveTask(void *param)
{
	...
	while(true)
	{
		...
		adc1_config_width(ADC_WIDTH_BIT_13);	//9, 10, 11, 12
		adc1_config_channel_atten(ADC1_CHANNEL_5, ADC_ATTEN_DB_11); //ADC_ATTEN_DB_0, ADC_ATTEN_DB_2_5, ADC_ATTEN_DB_6 ADC_ATTEN_DB_11	  
		recShift[currentRec] = phaseShift;
		unsigned long t0 = ESP.getCycleCount();
		for(int i = 0; i < depth; i++)
		{
			//int a = analogRead(34);
			int a = adc1_get_raw(ADC1_CHANNEL_5);
			sum += a;
